...
- 로컬 발표용으로 실행
- 기본값은 mock data이며, API 키가 있으면 Alpha Vantage 주가 API와 Google News RSS를 서버 라우트에서 조회
- KIS 인증 문제를 피하기 위해 미국 주식 일봉 API는 Alpha Vantage로 변경
- 실시간 WebSocket이 아닌 5분 간격 주기적 조회 방식
- 미국 주식과 S&P 500 중심
...
- 클라이언트 컴포넌트는 5분 간격으로 API route를 다시 호출
...
- `/`: 지수 봉차트와 3일 이동평균선, 시장 공통 뉴스, 종목 카드 목록, `/api/market` 주기 조회
...
- `npm run lint`, `npm run build`, `/api/market`, `/api/stocks/AAPL` 응답으로 확인합니다.
